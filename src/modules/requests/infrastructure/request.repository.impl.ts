...
export class RequestRepositoryImpl implements IRequestRepository {
    async createRescueRequest(data: CreateRescueRequestData): Promise<RescueRequest> {
        const response = await requestsApi.createRescueRequest(data);
        return (response as any).data;
    }

    async createEmergencyRequest(data: EmergencyRequestData): Promise<RescueRequest> {
