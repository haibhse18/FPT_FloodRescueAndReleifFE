...
import { Package, UserIcon, Van, WarehouseIcon } from "lucide-react";
...
import { vehicleApi } from "@/modules/vehicles";
...
function StatCard({ title, value, badge, positive = true, subtext, highlight, loading, accentColor }: StatCardProps) {
  return (
    <div
      style={{
        background: "#fff",
      ...
      <div style={{ display: "flex", gap: "16px", marginBottom: "24px", flexWrap: "wrap" }}>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {[
              { label: "Quản lý người dùng", icon: <UserIcon />, href: "/admin-users", color: "#1890ff", bg: "#e6f7ff" },
              { label: "Quản lý phương tiên", icon: <Van />, href: "/admin-system?tab=Phương%20tiện", color: "#722ed1", bg: "#f9f0ff" },
              { label: "Quản lý vật tư", icon: <Package />, href: "/admin-system?tab=Vật%20tư", color: "#722ed1", bg: "#f9f0ff" },
              { label: "Quản lý kho", icon: <WarehouseIcon />, href: "/admin-system?tab=Kho", color: "#fa8c16", bg: "#fff7e6" },
            ].map((item) => (
              <a
                key={item.label}
                href={item.href}
